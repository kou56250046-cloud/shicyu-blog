  ...
  return (
    <article className="max-w-3xl mx-auto">
      ...
      <nav className="text-xs text-stone-400 mb-6 flex items-center gap-1">
        <Link href="/" className="hover:text-indigo-600">鑑定一覧</Link>
        <span>/</span>
      ...
      <header className="bg-white rounded-xl border border-stone-200 p-6 mb-6">
        <div className="flex items-start justify-between gap-3 mb-4">
          ...
          {isCompatibility && (
            <span className="shrink-0 text-xs bg-rose-100 text-rose-600 px-2 py-1 rounded-full">
              相性鑑定
            </span>
          )}
        </div>
        ...
        <dl className="grid grid-cols-2 sm:grid-cols-3 gap-x-4 gap-y-2 text-sm mb-4">
          {!isCompatibility && (
            <>
              <InfoRow label="生年月日" value={reading.birthDate} />
              <InfoRow label="出生時刻" value={reading.birthTime} />
              <InfoRow label="性別" value={reading.gender} />
              <InfoRow label="続柄" value={reading.relationship} />
            </>
          )}
