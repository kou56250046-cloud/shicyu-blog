# 02 GitHub / Vercel デプロイ設定

## 概要
GitHub リポジトリを作成し、Vercel と連携して自動デプロイを設定する。
`main` ブランチへの push で Vercel が自動的にデプロイを行う。

---

## Todo

### GitHub
- [ ] GitHub でリポジトリを作成（例: `shicyu-blog`）
- [ ] ローカルで `git init` → `git remote add origin <URL>`
- [ ] 初回コミット・push を実行

### Vercel
- [ ] Vercel アカウントで「Add New Project」
- [ ] GitHub リポジトリを連携
- [ ] Framework Preset: Next.js を選択
- [ ] デプロイを実行し、発行された URL でアクセスできることを確認

### 動作確認
- [ ] `readings/` に md を追加して push → Vercel が自動デプロイされることを確認
- [ ] スマホからデプロイ URL にアクセスして表示を確認

---

## 参照
- CLAUDE.md「Git 運用ルール」セクション
- CLAUDE.md「ワークフロー Step 3」セクション
